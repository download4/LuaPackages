...
local function getMainThumbnailKeyframe(character, poseAssetId)
	local thumbnailKeyframe
	...
	if poseAssetId then
		...
		local thumbnailKeyframeNumber = module.GetNumberValueWithDefault(poseAsset,
			"ThumbnailKeyframe",
			nil)
		local rotationDegrees = module.GetNumberValueWithDefault(poseAsset,
...
module.SetPlayerCharacterPose = function(character, poseAssetId, confirmProceedAfterYield)
	...
	elseif humanoid.RigType == Enum.HumanoidRigType.R15 then
		...
		thumbnailKeyframe, givenPoseTrumpsToolPose = getMainThumbnailKeyframe(character, poseAssetId)
